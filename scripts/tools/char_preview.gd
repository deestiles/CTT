extends Node3D
## Character animation preview — load a Mixamo-rigged Synty character (with skin) and
## play a Mixamo action clip on it DIRECTLY (both share the same 48-bone skeleton, so
## no retargeting). Textures the mesh with the city atlas. Sky3D day/night + orbit cam.
##
## Env: CTT_CHAR (character fbx path), CTT_ANIM (action fbx path), CTT_SHOT=1 headless
## capture to user://char_preview.png, SHOT_TIME 0/1/2, SHOT_YAW/PITCH/ZOOM.
...
func _load_character() -> void:
	var char_path := "res://Assets/Animations/Character/SK_Character_Male_Police.fbx"
	if OS.has_environment("CTT_CHAR"): char_path = OS.get_environment("CTT_CHAR")
	var anim_path := "res://Assets/Animations/Walking_01.fbx"
	if OS.has_environment("CTT_ANIM"): anim_path = OS.get_environment("CTT_ANIM")
	...
	var walk := _extract_walk(anim_path)
	var ok := false
	if player and walk:
...
func _update_label() -> void:
	if _label:
		_label.text = "%s  ·  walk  ·  drag rotate · wheel zoom · T time" % TOD[_time_idx][0]
