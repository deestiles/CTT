...
## Assign the non-transition lane whose endpoint and heading continue the
## current transition curve. This preserves physical lane position through an
## intersection instead of searching left and changing lane index.
func assign_spatial_continuation(move_dir: MoveDir) -> bool:
	if not is_instance_valid(current_lane) or move_dir == MoveDir.STOP:
		return false
	var current_length := current_lane.curve.get_baked_length()
	var current_offset := current_length if move_dir == MoveDir.FORWARD else 0.0
	var current_near_offset := maxf(0.0, current_length - 0.5) if move_dir == MoveDir.FORWARD else minf(current_length, 0.5)
	var endpoint := current_lane.to_global(current_lane.curve.sample_baked(current_offset))
	var near_endpoint := current_lane.to_global(current_lane.curve.sample_baked(current_near_offset))
	var travel_heading := (endpoint - near_endpoint).normalized() if move_dir == MoveDir.FORWARD else (near_endpoint - endpoint).normalized()
	var required_lane_index := lane_index_from_identity(current_lane)
	var best_lane: RoadLane
	var best_score := INF
	var groups_checked: Array[StringName] = []
	var lanes: Array = []
	for container in road_manager.get_containers():
		if container.ai_lane_group in groups_checked:
			continue
		groups_checked.append(container.ai_lane_group)
		lanes.append_array(get_tree().get_nodes_in_group(container.ai_lane_group))
	for candidate in lanes:
		if not candidate is RoadLane or candidate == current_lane or candidate.transition:
			continue
		var lane_candidate := candidate as RoadLane
		if required_lane_index >= 0 and lane_index_from_identity(lane_candidate) != required_lane_index:
			continue
		var candidate_length: float = lane_candidate.curve.get_baked_length()
		if candidate_length < 0.5:
			continue
		# Prefab turn lanes can merge into the middle of an authored through-lane,
		# so compare against its closest point rather than only its endpoint.
		var candidate_local_closest := lane_candidate.curve.get_closest_point(lane_candidate.to_local(endpoint))
		var candidate_offset: float = lane_candidate.curve.get_closest_offset(candidate_local_closest)
		var candidate_endpoint: Vector3 = lane_candidate.to_global(candidate_local_closest)
		var endpoint_distance: float = endpoint.distance_to(candidate_endpoint)
		if endpoint_distance > 1.25:
			continue
		var candidate_near_offset := minf(candidate_length, candidate_offset + 0.5) if move_dir == MoveDir.FORWARD else maxf(0.0, candidate_offset - 0.5)
		var candidate_near: Vector3 = lane_candidate.to_global(lane_candidate.curve.sample_baked(candidate_near_offset))
		var candidate_heading: Vector3 = (candidate_near - candidate_endpoint).normalized()
		var alignment := travel_heading.dot(candidate_heading)
		if alignment < 0.65:
			continue
		var score := endpoint_distance + (1.0 - alignment) * 2.0
		if score < best_score:
			best_score = score
			best_lane = lane_candidate
	if not is_instance_valid(best_lane):
		return false
	assign_lane(best_lane)
	return true


## At a lane end with no authored forward link, select a transition lane whose
## entrance shares the same physical endpoint. This lets traffic turn through
## the intersection instead of driving beyond the generated road or stopping.
func assign_available_turn(move_dir: MoveDir) -> bool:
	if not is_instance_valid(current_lane) or move_dir != MoveDir.FORWARD:
		return false
	var current_length := current_lane.curve.get_baked_length()
	var endpoint := current_lane.to_global(current_lane.curve.sample_baked(current_length))
	var near_endpoint := current_lane.to_global(current_lane.curve.sample_baked(maxf(0.0, current_length - 0.75)))
	var approach_heading := (endpoint - near_endpoint).normalized()
	var required_lane_index := lane_index_from_identity(current_lane)
	var best_lane: RoadLane
	var best_score := INF
	var groups_checked: Array[StringName] = []
	...
	for container in road_manager.get_containers():
		...
		lanes.append_array(get_tree().get_nodes_in_group(container.ai_lane_group))
	for candidate in lanes:
		if not candidate is RoadLane or candidate == current_lane:
			continue
		var turn_lane := candidate as RoadLane
		if required_lane_index >= 0 and lane_index_from_identity(turn_lane) != required_lane_index:
			continue
		var candidate_length := turn_lane.curve.get_baked_length()
		if candidate_length < 1.0:
			continue
		var entrance := turn_lane.to_global(turn_lane.curve.sample_baked(0.0))
		var entrance_distance := endpoint.distance_to(entrance)
		if entrance_distance > 1.6:
			continue
		var candidate_ahead := turn_lane.to_global(turn_lane.curve.sample_baked(minf(1.0, candidate_length)))
		var exit_heading := (candidate_ahead - entrance).normalized()
		var alignment := approach_heading.dot(exit_heading)
		if alignment < -0.2:
			continue
		# Prefer a connected transition, then the closest and smoothest entrance.
		var transition_penalty := 0.0 if turn_lane.transition else 1.5
		var score := entrance_distance * 4.0 + transition_penalty + absf(alignment - 0.25)
		if score < best_score:
			best_score = score
			best_lane = turn_lane
	if not is_instance_valid(best_lane):
...
## The prefab naming/tag convention numbers lanes outward from the divider:
## 0 is inside and 1 is outside. Turn paths such as N1W1 carry the same index.
func lane_index_from_identity(lane: RoadLane) -> int:
	for identity in [lane.lane_next_tag, lane.lane_prior_tag, String(lane.name)]:
		var identity_text := String(identity)
		if identity_text.is_empty():
			continue
		var final_character := identity_text.right(1)
		if final_character.is_valid_int():
			return final_character.to_int()
	return -1
